...
def run(args):
    ...
    model = get_multigrain(args.backbone, p=args.init_pooling_exponent, include_sampling=False,
                           pretrained_backbone=args.pretrained_backbone, learn_p=True)
    print("Multigrain model with {} backbone and p={} pooling:".format(args.backbone, args.init_pooling_exponent))
    print(model)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--kind', choices=['imagenet-val', 'imagenet-trainaug', 'holidays', 'copydays', 'ukbench'],
                        default='holidays', help='which evaluation to make')
    parser.add_argument('--expdir', default='experiments/resnet50/finetune500/holidays500', help='evaluation destination directory')
    parser.add_argument('--resume-epoch', default=-1, type=int, help='resume epoch (-1: last, 0: from scratch)')
    parser.add_argument('--resume-from', default='experiments/resnet50/finetune500', help='resume checkpoint file/folder')
